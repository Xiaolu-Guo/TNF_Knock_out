function [peak_times, peak_amps, valley_times, valley_amps, fig1] = nfkbpeaks_new(trajectories, baseline, varargin)
%% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
%   input non baseline deducted data to input into globalpeaks_new
% INPUTS (required)
...
for i = 1:size(peak_times,1)
    ...
    if sum(~isnan(vect)) > 10
        [pks, locs, heights] = globalpeaks_new(vect, baseline(i), num_peaks);
    else
        pks = []; locs = []; heights = [];
    end
    ...
    drops = (locs<begin_frame) | (heights<min_height);  %%% can subtract baseline from peak amplitute to create another filter AS6/19/2020
    pks(drops) = [];
    locs(drops) = [];
    if ~isempty(locs)
